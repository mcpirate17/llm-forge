...
unrelated to their diffs* is demoted automatically. "Unrelated to the diff" is
mechanised, not judged: a blocking finding is unrelated when its `path` is outside
the PR's changed files. That is the exact shape of the jscpd failure -- 600 duplicate
pairs in aria C++ that the candidate never touched -- and of the waiver-base failure,
where every finding named a test file the PR did not open.
...
def findings_are_unrelated_to_diff(
    findings: list[dict[str, object]], changed_paths: set[str]
) -> bool:
    """True when no blocking finding names a path the PR actually changed.

    A finding with no path (a `governance`-scoped one) counts as unrelated: the
    candidate cannot have caused it by editing a file. This is the jscpd shape --
    600 duplicate pairs in code the PR never opened.
    """
    blocking = [
        finding
        for finding in findings
        if finding.get("severity") in ("critical", "high") and not finding.get("exception_id")
    ]
    if not blocking:
        return False
    return not any(
        isinstance(finding.get("path"), str) and finding["path"] in changed_paths
...
def _gh_api(args: list[str]) -> str:
    ...
    if completed.returncode != 0:
        raise RolloutError(f"gh api {' '.join(args)} failed: {completed.stderr.strip()}")
...
def set_ruleset_required_checks(repo: str, ruleset_id: int, contexts: list[str]) -> None:
    """Rewrite the ruleset's required_status_checks rule to exactly `contexts`."""
    payload = json.loads(_gh_api([f"repos/{repo}/rulesets/{ruleset_id}"]))
    rules = [rule for rule in payload.get("rules", []) if rule.get("type") != "required_status_checks"]
    if contexts:
        rules.append(
            {
    ...
    body = {"name": payload["name"], "enforcement": payload["enforcement"], "rules": rules}
    if payload.get("conditions"):
        body["conditions"] = payload["conditions"]
    completed = subprocess.run(
        ["gh", "api", "--method", "PUT", f"repos/{repo}/rulesets/{ruleset_id}", "--input", "-"],
...
def command_status(args: argparse.Namespace) -> int:
    ...
    for check in checks:
        ...
        print(f"{check}:")
        print(f"  promotion: {'ELIGIBLE' if promote_ok else 'blocked'} -- {promote_reason}")
        print(f"  demotion:  {'REQUIRED' if demote_needed else 'not needed'} -- {demote_reason}")
    return 0
...
def command_record(args: argparse.Namespace) -> int:
    ...
    print(f"recorded {args.check} on #{args.pr}: {args.conclusion} (required={args.required})")
    return 0
...
def command_promote(args: argparse.Namespace) -> int:
    ...
    if not args.acknowledge_owner_decision:
        ...
        return 1
    contexts = sorted(set(ruleset_required_checks(args.repo, args.ruleset)) | {args.check})
...
def command_demote(args: argparse.Namespace) -> int:
    contexts = [c for c in ruleset_required_checks(args.repo, args.ruleset) if c != args.check]
    set_ruleset_required_checks(args.repo, args.ruleset, contexts)
    append_handoff(
...
def command_audit(args: argparse.Namespace) -> int:
    ...
    for check in checks:
        ...
        if args.apply:
            contexts = [c for c in ruleset_required_checks(args.repo, args.ruleset) if c != check]
            set_ruleset_required_checks(args.repo, args.ruleset, contexts)
            append_handoff(f"{check} auto-demoted from required", reason)
            print(f"  demoted {check} on ruleset {args.ruleset} and wrote a handoff entry")
    if exit_code == 0:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    status = subparsers.add_parser("status", help="rollout state for one or every check")
    status.add_argument("--check", default=None)
    status.set_defaults(func=command_status)

    record = subparsers.add_parser("record", help="record one check verdict on a merged PR")
    record.add_argument("--check", required=True)
    record.add_argument("--pr", type=int, required=True)
    record.add_argument("--conclusion", required=True, choices=("success", "failure", "cancelled", "skipped"))
    ...
    promote = subparsers.add_parser("promote", help="make a check required (owner decision)")
    ...
    audit = subparsers.add_parser("audit", help="demote any check meeting the demotion condition")
