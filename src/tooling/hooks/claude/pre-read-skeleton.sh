...
REPO_ROOT="${PROJECT_DIR:-$(cd "$HERE/../../.." && pwd)}"
# shellcheck source=/dev/null
source "$HERE/_runtime.sh"
hook_pythonpath "$REPO_ROOT"
PYTHON="$(hook_python "$REPO_ROOT")"
cd "$REPO_ROOT"
"$PYTHON" "$HERE/_pre_read_skeleton.py" \
  | "$PYTHON" -m conductor.context_telemetry hook-context --hook pre-read-skeleton
