...
- PR #51 isolated bytecode caches by refusing to write any (retention campaign 51.1 s -> 131.8 s); #52 recovers wall time: 51.12 s (median of 3; #51-as-merged measured 127.67 s, same 90/8/19 outcomes and survivor set) by caching unmutated bytecode in a per-run prefix and evicting only the mutated file.
